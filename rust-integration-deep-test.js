...
import { RustExecutor, createRustExecutor, initLogging } from './src/core/execution/rust-executor/rust-native-module.js';
...
async function deepTestRustIntegration() {
  try {
    ...
    console.log('    🔬 Analysis: Success =', existsResult.success, ', Has actual result =', !!existsResult.result);
    ...
    console.log('    🔬 Analysis: Success =', listResult.success, ', Has actual result =', !!listResult.result);
    ...
    console.log('    🔬 Analysis: Success =', readResult.success, ', Has actual content =', 
    ...
    console.log('    🔬 Analysis: Success =', echoResult.success, ', Contains expected output =', 
    ...
    console.log('    🔬 Analysis: Success =', lsResult.success, ', Has directory listing =', 
                JSON.stringify(lsResult.result).includes('package.json'));
    
    // 3. Test Generic Tool Execution
    console.log('\n🔧 GENERIC TOOL TESTS:');
    
    const genericResult = executor.execute('filesystem', JSON.stringify({
      operation: 'exists',
      path: '.'
    }), {
      securityLevel: 'medium',
    ...
    const benchmarkResult = executor.benchmark_execution ? 
      executor.benchmark_execution(100) : 'Benchmark not available';
