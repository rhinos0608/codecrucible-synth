...
import {
  ...
  SmitheryServer,
} from '../../mcp-servers/smithery-registry-integration.js';
...
export class AdvancedMCPDiscoverySystem extends EventEmitter {
  ...
  async discoverServers(forceRefresh: boolean = false): Promise<DiscoveredMCPServer[]> {
    ...
    try {
      ...
      logger.info(
        `Discovered ${allServers.length} MCP servers from ${this.registryClients.size} sources`
      );
  ...
  private inferComplexity(inputSchema: any): 'low' | 'medium' | 'high' {
    if (!inputSchema?.properties) return 'low';
  ...
  private estimatePerformance(server: SmitheryServer): number {
    ...
    const complexityPenalty =
      server.tools.filter(tool => this.inferComplexity(tool.inputSchema) === 'high').length * 5;
  ...
  private inferCategories(server: SmitheryServer): string[] {
    const categories = new Set(
      server.tools.map(tool => this.inferCategory(tool.name, tool.description))
    );
  ...
  getServerRecommendations(
    requirements: ServerDiscoveryQuery,
    limit: number = 5
  ...
  getDiscoveryStats() {
    ...
    return {
      ...
      byCategory: this.groupBy(
        servers.flatMap(s => s.categories.map(cat => ({ category: cat }))),
        item => item.category
      ),
