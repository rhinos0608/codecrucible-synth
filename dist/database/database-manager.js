...
export class DatabaseManager {
        ...
        try {
            if (this.config.inMemory) {
                ...
                this.db = new Database(dbPath, {
                    ...
                    fileMustExist: false,
                    timeout: 5000,
                    verbose: process.env.NODE_ENV === 'development' ? (message, ...additionalArgs) => {
    ...
    async storeVoiceInteraction(interaction) {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `);
        const result = stmt.run(interaction.sessionId, interaction.voiceName, interaction.prompt, interaction.response, interaction.confidence, interaction.tokensUsed);
    ...
    async storeCodeAnalysis(analysis) {
      ...
      VALUES (?, ?, ?, ?, ?)
    `);
        const result = stmt.run(analysis.projectId, analysis.filePath, analysis.analysisType, JSON.stringify(analysis.results), analysis.qualityScore || null);
        return result.lastInsertRowid;
    }
    /**
     * Get project by path
