...
export class OllamaProvider implements LLMProvider {
  ...
  private buildPrompt(prompt: string, taskType: string): string {
    const systemPrompts: Record<string, string> = {
      analysis: 'You are an expert code analyst. Provide thorough, detailed analysis with specific insights and recommendations.',
      security: 'You are a security expert. Analyze code for vulnerabilities, security issues, and provide detailed security recommendations.',
      architecture: 'You are a software architect. Design robust, scalable solutions with clear architectural decisions and trade-offs.',
      'multi-file': 'You are working with multiple files. Consider relationships between files and provide comprehensive solutions.',
      debugging: 'You are a debugging expert. Identify issues systematically and provide detailed troubleshooting steps.',
      review: 'You are conducting a code review. Provide detailed feedback on code quality, performance, and best practices.',
      default: 'You are an expert software engineer. Provide thoughtful, well-reasoned solutions with detailed explanations.'
    };
  ...
  private calculateConfidence(content: string, responseTime: number, taskType?: string): number {
    ...
    if (content.includes('Analysis:') || content.includes('Recommendation:') || 
        content.includes('Issue:') || content.includes('Solution:')) {
  ...
  getCapabilities(): LLMCapabilities {
    return {
      strengths: ['analysis', 'reasoning', 'security', 'architecture', 'complex-tasks', 'multi-file'],
      optimalFor: ['code-analysis', 'security-review', 'architecture-design', 'debugging', 'code-review'],
      responseTime: '5-30s',
      contextWindow: 8192,
