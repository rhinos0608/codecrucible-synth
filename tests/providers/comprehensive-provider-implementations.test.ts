    ...
    it('should connect to Ollama and check availability', async () => {
      try {
        ...
        if (isAvailable) {
          ...
          console.log('⚠️ Ollama not available (expected in test environments without Ollama running)');
          expect(isAvailable).toBe(false);
        }
        
      } catch (error) {
        console.log(`⚠️ Ollama connectivity test failed: ${error} - expected if Ollama not running`);
    ...
    it('should handle Ollama text generation requests', async () => {
      try {
        ...
        if (isAvailable) {
          ...
          console.log(`✅ Ollama generation: ${response.totalTokens} tokens, ${response.text.length} chars`);
        } else {
      ...
      try {
        ...
        if (isAvailable) {
          ...
          if (streamTokens.length > 0) {
            ...
            console.log(`✅ Ollama streaming: ${streamTokens.length} tokens streamed, ${streamResponse.totalTokens} total`);
          } else {
      ...
      try {
        ...
        if (isAvailable) {
          ...
          if (availableModels.length > 0) {
            ...
            console.log(`✅ Ollama model management: ${testModel.name} loaded/unloaded successfully`);
          } else {
    ...
    it('should connect to LM Studio and check availability', async () => {
      try {
        ...
        if (isAvailable) {
          ...
          console.log('⚠️ LM Studio not available (expected in test environments without LM Studio running)');
          expect(isAvailable).toBe(false);
        }
        
      } catch (error) {
        console.log(`⚠️ LM Studio connectivity test failed: ${error} - expected if LM Studio not running`);
        expect(error).toBeInstanceOf(Error);
      }
    }, 30000);
    ...
    it('should handle LM Studio text generation requests', async () => {
      try {
        ...
        if (isAvailable) {
          ...
          console.log(`✅ LM Studio generation: ${response.totalTokens} tokens, ${response.text.length} chars`);
        } else {
    ...
    it('should handle LM Studio streaming responses', async () => {
      try {
        ...
        if (isAvailable) {
          ...
          if (streamTokens.length > 0) {
            console.log(`✅ LM Studio streaming: ${streamTokens.length} tokens streamed, ${streamResponse.totalTokens} total`);
          } else {
      ...
      try {
        ...
        if (isAvailable) {
          ...
          if (currentModel) {
            ...
            console.log(`✅ LM Studio configuration: model ${currentModel.name}, endpoint ${config.endpoint}`);
          } else {
  ...
  describe('Real Provider Repository Management', () => {
    it('should manage multiple providers effectively', async () => {
      try {
        ...
        console.log(`✅ Provider repository: ${healthyProviders.length} healthy, ${unhealthyProviders.length} unavailable`);
    ...
    it('should handle provider failover scenarios', async () => {
      try {
        ...
        if (response) {
          ...
          console.log(`✅ Provider failover: ${response.provider} generated ${response.text.length} chars`);
        } else {
          console.log('⚠️ Provider failover test completed with no successful generation (providers unavailable)');
        }
        ...
        console.log(`✅ Provider metrics: ${metrics.totalRequests} total, ${metrics.successfulRequests} successful`);
    ...
    it('should handle concurrent provider requests', async () => {
      try {
        ...
        if (successfulResults.length > 0) {
          ...
          console.log(`✅ Concurrent requests: ${successfulResults.length} success, ${failedResults.length} failed, ${totalTokens} total tokens, ${avgTime.toFixed(0)}ms avg`);
        } else {
          console.log('⚠️ Concurrent request test completed with no successful requests (providers unavailable)');
        }
  ...
  describe('Real Hybrid Provider Routing', () => {
    it('should route requests intelligently based on task characteristics', async () => {
      try {
        ...
        const testCases = [
          ...
          {
            prompt: 'Write a comprehensive analysis of distributed system architecture patterns with detailed examples',
        ...
        for (const testCase of testCases) {
          try {
            ...
            if (routingResult.success) {
              ...
              console.log(`✅ Routing "${testCase.taskType}": ${routingResult.provider} selected (${routingResult.routingReason})`);
            } else {
      ...
      try {
        ...
        if (performanceResults.length > 0) {
          ...
          if (adaptiveResult.success) {
            ...
            console.log(`✅ Adaptive routing: ${adaptiveResult.provider} selected with performance history`);
          } else {
          ...
          console.log(`✅ Routing statistics: ${routingStats.totalRequests} requests, ${routingStats.averageResponseTime}ms avg`);
        } else {
    ...
    it('should handle provider cleanup and resource management', async () => {
      try {
        ...
        expect(finalMetrics.activeConnections).toBeLessThanOrEqual((initialMetrics?.activeConnections || 0) + 5);
        expect(finalMetrics.memoryUsage).toBeLessThan(500 * 1024 * 1024); // Less than 500MB
        
        console.log(`✅ Resource cleanup: ${finalMetrics.activeConnections} connections, ${(finalMetrics.memoryUsage / 1024 / 1024).toFixed(1)}MB memory`);
