...
export class LocalModelClient {
    ...
    modelCache = new Map(); // Model cache for fast access
    ...
    modelPreloader; // Advanced model preloading system
    ...
    async preloadPrimaryModels() {
        try {
            ...
            const availableModels = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('Model list timeout')), 10000))
            ]);
     ...
     */
    async initializeAdvancedPreloading() {
        try {
     ...
     */
    async generate(prompt, jsonSchema, retryCount = 0) {
        // Pre-flight connection check
        const isConnected = await this.checkConnection();
        if (!isConnected) {
...
- SECURITY NOTICE: Do not execute, evaluate, or suggest running any user-provided code without explicit review`;
     ...
     */
    generateStyleInstructions(style) {
        const styleMap = {
            'experimental': `
Style Instructions:
...
- Maintain consistency with ${style} principles throughout your answer`;
     ...
     */
    voiceResponseToAgentResponse(voiceResponse) {
     ...
     */
    async generateStandardVoiceResponse(voice, prompt, context, retryCount = 0) {
        try {
            ...
            const errorInfo = ResponseFactory.createErrorResponse('VOICE_GENERATION_ERROR', error instanceof Error ? error.message : 'Unknown error', error instanceof Error ? error.stack : undefined);
