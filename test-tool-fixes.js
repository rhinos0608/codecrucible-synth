...
async function testModelSelection() {
  ...
  try {
    ...
    if (result.selectedModel.name.includes('llama3.1')) {
      ...
      console.log(
        '   ⚠️  OK: Qwen2.5-Coder 3B selected but 7B+ would be better for function calling'
      );
    } else {
...
async function testCapabilityDetection() {
  ...
  for (const model of testModels) {
    try {
      ...
      console.log(`${model}:`);
      console.log(
        `  Function Calling: ${capabilities.functionCalling ? '✅' : '❌'} (confidence: ${capabilities.confidence})`
      );
...
async function main() {
  ...
  console.log(
    '6. ✅ Prioritized proven function calling models: Llama 3.1 > Qwen2.5-Coder 7B+ > others\n'
  );
