  ...
  async isAvailable(): Promise<boolean> {
    try {
      ...
      const timeoutId = setTimeout(
        () => controller.abort(),
        parseInt(process.env.OLLAMA_HEALTH_CHECK_TIMEOUT || '5000')
      );
  ...
  private async makeRequest(prompt: string, options: any): Promise<any> {
    ...
    if (options.tools && options.tools.length > 0) {
      ...
      const requestObj = {
        ...
        num_ctx:
          options.num_ctx || options.contextLength || this.getContextLength(options.taskType),
        options: {
      ...
      options: {
        ...
        ...Object.fromEntries(
          Object.entries(options).filter(([key]) =>
            ['mirostat', 'mirostat_eta', 'mirostat_tau', 'repeat_penalty', 'tfs_z'].includes(key)
          )
        ),
      },
    };
