...
export interface ProfileOperation {
  id: string;
  name: string;
  category: 'llm_inference' | 'tool_execution' | 'prompt_preparation' | 'event_bus' | 'cache_operation';
  startTime: number;
  endTime?: number;
  duration?: number;
...
export class PerformanceProfiler extends EventEmitter {
  ...
  private initializeProfilerMetrics(): void {
    ...
    this.metricsCollector.registerGauge('active_profiling_sessions', 'Currently active profiling sessions');
    this.metricsCollector.registerCounter('slow_operations_total', 'Total slow operations detected', ['category', 'threshold']);
  }
  ...
  endOperation(sessionId: string, operationId: string, error?: Error): ProfileOperation {
    ...
    switch (operation.category) {
      case 'llm_inference':
        this.metricsCollector.observeHistogram('llm_inference_detailed_duration_seconds', duration, labels);
        break;
      case 'tool_execution':
        this.metricsCollector.observeHistogram('tool_execution_detailed_duration_seconds', duration, labels);
        break;
      case 'prompt_preparation':
        this.metricsCollector.observeHistogram('prompt_preparation_duration_seconds', duration, labels);
        break;
      case 'event_bus':
    ...
    if (this.config.enableDetailedLogging) {
      logger.debug(`Completed operation: ${operation.name} (${operationId}) in ${duration.toFixed(3)}s`);
    }
  ...
  getPerformanceInsights(): {
    ...
    return {
      ...
      categoryAverages,
      slowOperationCount: recent.filter(op => 
        op.duration && op.duration * 1000 > this.config.slowOperationThresholdMs
      ).length,
    };
  }
