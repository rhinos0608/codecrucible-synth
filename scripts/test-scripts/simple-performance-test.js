...
class SimplePerformanceTest {
    ...
    displayTestResult(result) {
        ...
        if (result.commandsExecuted) {
            console.log(`   Commands: ${result.commandsSuccessful}/${result.commandsExecuted} successful`);
            console.log(`   Average time: ${result.averageTime}ms`);
        }
        
        if (result.averageTime && result.p95Time) {
            console.log(`   Avg: ${result.averageTime}ms, P95: ${result.p95Time}ms, Min: ${result.minTime}ms, Max: ${result.maxTime}ms`);
        }
        
        if (result.initialMemoryMB) {
            console.log(`   Memory: ${result.initialMemoryMB}MB → ${result.finalMemoryMB}MB (Peak: ${result.peakHeapUsedMB}MB)`);
            console.log(`   Memory increase: ${result.memoryIncreaseMB}MB, Leak detected: ${result.memoryLeakDetected}`);
        }
        ...
        if (result.maxSuccessfulConcurrency !== undefined) {
            ...
            result.concurrencyResults.forEach(cr => {
                console.log(`   Level ${cr.level}: ${cr.successCount}/${cr.level} success (${(cr.successRate * 100).toFixed(1)}%)`);
            });
        }
