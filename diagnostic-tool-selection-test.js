...
const testPrompts = [
  ...
  // Research tasks  
  {
    prompt: "Research the latest trends in AI CLI tools and find documentation about best practices",
...
async function runDiagnostic() {
  ...
  const mockTools = [
    { function: { name: "filesystem_read_file" }, name: "filesystem_read_file", description: "Read file contents" },
    { function: { name: "filesystem_write_file" }, name: "filesystem_write_file", description: "Write file contents" },
    { function: { name: "filesystem_list_directory" }, name: "filesystem_list_directory", description: "List directory contents" },
    { function: { name: "filesystem_find_files" }, name: "filesystem_find_files", description: "Find files by pattern" },
    { function: { name: "mcp_execute_command" }, name: "mcp_execute_command", description: "Execute system commands" },
    { function: { name: "mcp_read_file" }, name: "mcp_read_file", description: "Read files via MCP" },
    { function: { name: "mcp_list_directory" }, name: "mcp_list_directory", description: "List directories via MCP" },
    { function: { name: "mcp_plan_request" }, name: "mcp_plan_request", description: "Create task plans" },
    { function: { name: "mcp_get_next_task" }, name: "mcp_get_next_task", description: "Get next task from plan" }
  ];
  ...
  for (const test of testPrompts) {
    ...
    try {
      ...
      console.log(`   ✓ Detected domain: ${analysis.primaryDomain} (confidence: ${(analysis.confidence * 100).toFixed(1)}%)`);
      ...
      console.log(`   ✓ Selected ${selectedToolNames.length} tools: ${selectedToolNames.join(', ')}`);
      console.log(`   ✓ Reasoning: ${toolResult.reasoning}`);
      
      // Evaluate results
      let domainMatch = analysis.primaryDomain === test.expectedDomain || test.expectedDomain === 'mixed';
      let toolMatch = test.expectedTools.some(expectedTool => selectedToolNames.includes(expectedTool));
      ...
      if (domainMatch && toolMatch && selectedToolNames.length > 0) {
        ...
        if (!domainMatch) console.log(`      - Domain mismatch: expected ${test.expectedDomain}, got ${analysis.primaryDomain}`);
  ...
  try {
    ...
    const mockModelClient = {
      ...
      synthesize: async (options) => ({ content: `Mock synthesis for: ${options.prompt.substring(0, 50)}...` })
    };
