...
export class RustStreamingClient extends EventEmitter {
  ...
  async streamCommand(
    ...
    logger.info(`🚀 Starting Rust command stream: ${command} ${args.join(' ')} (session: ${sessionId})`);
    session.state = 'streaming';
  ...
  /**
   * Handle incoming chunks from Rust
   */
  private async handleChunk(sessionId: string, rawChunk: any): Promise<void> {
    ...
    if (!session || !processor) {
      logger.warn(`Received chunk for unknown session: ${sessionId}`);
      return;
    }
    ...
    try {
      ...
      session.stats.avgChunkTime = (session.stats.avgChunkTime * (session.stats.chunksProcessed - 1) + chunkTime) / session.stats.chunksProcessed;
      
      // Update peak memory if available
      if (chunk.resourceUsage?.memoryBytes && chunk.resourceUsage.memoryBytes > session.stats.peakMemoryBytes) {
