...
export class DomainAwareToolOrchestrator {
    ...
    const domains: ToolDomain[] = [
      {
        ...
        keywords: [
          // File operations
          'read', 'write', 'file', 'directory', 'folder', 'create', 'delete', 'modify', 'edit',
          // Code-related
          'code', 'function', 'class', 'variable', 'debug', 'error', 'bug', 'refactor',
          'implement', 'build', 'compile', 'test', 'syntax', 'programming', 'develop',
          // Languages
          'javascript', 'typescript', 'python', 'java', 'cpp', 'rust', 'go', 'html', 'css',
          // Development
          'repository', 'git', 'commit', 'branch', 'merge', 'deploy', 'package', 'dependency'
        ],
        tools: [
      ...
      },
      {
        ...
        keywords: [
          'research', 'search', 'find', 'discover', 'investigate', 'analyze', 'study',
          'information', 'data', 'facts', 'learn', 'explore', 'understand', 'explain',
          'what is', 'how to', 'why', 'when', 'where', 'who', 'compare', 'evaluate',
          'documentation', 'manual', 'guide', 'tutorial', 'help', 'lookup'
        ],
        tools: [
      ...
      },
      {
        ...
        keywords: [
          'system', 'process', 'service', 'daemon', 'status', 'monitor', 'performance',
          'memory', 'cpu', 'disk', 'network', 'install', 'configure', 'setup',
          'environment', 'path', 'variable', 'permission', 'user', 'admin', 'sudo',
          'command', 'terminal', 'shell', 'bash', 'execute', 'run', 'launch'
        ],
        tools: [
          'mcp_execute_command',
          'mcp_remote_execute',
      ...
      },
      {
        ...
        keywords: [
          'plan', 'task', 'todo', 'organize', 'manage', 'schedule', 'priority',
          'workflow', 'process', 'project', 'milestone', 'deadline', 'timeline',
          'track', 'progress', 'status', 'complete', 'finish', 'done',
          'strategy', 'approach', 'methodology', 'steps', 'phases'
        ],
        tools: [
          'mcp_plan_request',
          'mcp_get_next_task', 
      ...
      },
      {
        ...
        keywords: [
          'complex', 'multiple', 'various', 'different', 'several', 'both',
          'comprehensive', 'full', 'complete', 'everything', 'all',
          'analysis and', 'research and', 'build and', 'create and'
        ],
        tools: [
          'filesystem_read_file',
          'filesystem_write_file',
  ...
  private buildKeywordIndex(): void {
    ...
    logger.debug('Keyword index built', {
      ...
      avgDomainsPerKeyword: Array.from(this.keywordIndex.values()).reduce((sum, domains) => sum + domains.length, 0) / this.keywordIndex.size
    });
  }
   ...
   */
  getToolsForPrompt(prompt: string, allAvailableTools: any[]): {
    tools: any[];
    ...
    const reasoning = `Domain: ${analysis.primaryDomain} (${(analysis.confidence * 100).toFixed(0)}% confidence), ` +
   ...
   */
  private getToolsForDomain(domainName: string, allAvailableTools: any[], analysis: DomainAnalysis): {
    tools: any[];
    analysis: DomainAnalysis;
    reasoning: string;
  } {
