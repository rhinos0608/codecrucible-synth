...
export class CLIDisplay {
   ...
   */
  static async displayStreamingResponse(request: ExecutionRequest, streamingClient: StreamingAgentClient): Promise<void> {
    let buffer = '';
  ...
  static async showModelRecommendations(): Promise<void> {
    ...
    models.forEach(model => {
      const perfColor = model.performance === 'Very High' ? 'green' : 
                        model.performance === 'High' ? 'yellow' : 'gray';
