...
export class EnhancedStartupIndexer {
  ...
  async indexProject(): Promise<ProjectIndex> {
    ...
    console.log(
      picocolors.gray(
        `   ${Object.keys(files).length} files, ${Object.keys(documentation).length} docs`
      )
    );
  ...
  private determineFileType(filePath: string): ProjectFile['type'] {
    ...
    if (
      ['.ts', '.js', '.tsx', '.jsx', '.py', '.go', '.rs', '.java', '.cpp', '.c'].some(ext =>
        name.endsWith(ext)
      )
    ) {
      return 'source';
    ...
    if (
      ['.css', '.scss', '.less', '.png', '.jpg', '.jpeg', '.gif', '.svg', '.ico'].some(ext =>
        name.endsWith(ext)
      )
    ) {
      return 'asset';
  ...
  private extractImports(filePath: string, content: string): string[] {
    ...
    const importMatches = [...content.matchAll(/import\s+(?:{\s*([^}]+)\s*}|(\w+))/g)];
  ...
  private extractCodeBlocks(
    content: string
  ): Array<{ language: string; code: string; lineNumber: number }> {
  ...
  private async buildMetadata(
    ...
    return {
      ...
      type: existsSync(packageJsonPath)
        ? 'npm'
  ...
  private buildSearchIndices(files: Record<string, ProjectFile>): ProjectIndex['searchIndex'] {
    ...
    const fuseIndex = new Fuse(fileArray, {
      keys: ['relativePath', 'language', 'type', 'exports', 'imports'],
      threshold: 0.3,
      includeScore: true,
    });
  ...
  private async performAnalysis(
    ...
    const highestComplexity = complexities.sort((a, b) => b.complexity - a.complexity).slice(0, 10);
