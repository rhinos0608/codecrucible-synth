...
export class AlertManager {
  ...
  exportData(timeRange?: { start: Date; end: Date }): Alert[] {
    ...
    return this.history.filter(a => a.triggeredAt >= timeRange.start && a.triggeredAt <= timeRange.end);
  }

  getStats(): AlertStats {
    const recent = this.history.filter(a => a.triggeredAt > new Date(Date.now() - 24 * 60 * 60 * 1000));
    return {
      totalRules: this.rules.size,
      activeAlerts: this.active.size,
      alertsLast24h: recent.length,
      criticalAlerts: Array.from(this.active.values()).filter(a => a.severity === 'critical').length,
