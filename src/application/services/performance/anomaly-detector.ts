...
export class PerformanceAnomalyDetector extends EventEmitter implements IPerformanceAnomalyDetector {
  ...
  private detectTrendAnomalies(metrics: SystemMetrics): PerformanceAnomalyEvent[] {
    ...
    if (cpuTrend && cpuTrend.direction === 'INCREASING' && cpuTrend.rate > 5) { // 5% per measurement
      anomalies.push(this.createAnomaly(
        'PREDICTION_WARNING',
        'MEDIUM',
        'cpu_usage_trend',
    ...
    if (memoryTrend && memoryTrend.direction === 'INCREASING' && memoryTrend.rate > 3) { // 3% per measurement
      anomalies.push(this.createAnomaly(
        'PREDICTION_WARNING',
        'MEDIUM',
        'memory_usage_trend',
  ...
  private updateBaselines(): void {
    ...
    // Update CPU baseline
    this.updateMetricBaseline('cpu_usage', recentMetrics.map(m => m.cpu.usage));
    
    // Update memory baseline
    this.updateMetricBaseline('memory_usage', recentMetrics.map(m => m.memory.usage));
    
    // Update GC baseline
    this.updateMetricBaseline('gc_pause_time', recentMetrics.map(m => m.gc.pauseTime));
    
    // Update network baseline
    this.updateMetricBaseline('network_latency', recentMetrics.map(m => m.network.latencyP95));
  }
  ...
  private calculateTrend(metricName: string): MetricTrend | null {
    ...
    const confidence = Math.min(1, Math.abs(slope) / (values.reduce((sum, val) => sum + val, 0) / n) * 10);
  ...
  private projectValue(trend: MetricTrend, periodsAhead: number): number {
    const currentValue = this.metricsHistory.length > 0 
  ...
  private updateAnomalyPatterns(anomaly: PerformanceAnomalyEvent): void {
    ...
    pattern.avgSeverity = (pattern.avgSeverity * (pattern.frequency - 1) + severityValue) / pattern.frequency;
