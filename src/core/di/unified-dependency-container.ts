...
export class UnifiedDependencyContainer extends EventEmitter {
  ...
  private resourceCoordinator = new Map<string, {
    locks: Set<string>;
  ...
  getContainerStatus(): {
    ...
    return {
      ...
      resourceLocks: Array.from(this.resourceCoordinator.values()).reduce((acc, c) => acc + c.locks.size, 0),
      services
    };
  }
  ...
  private async createInstance<T extends IService>(node: DependencyNode<T>, scope?: string): Promise<T> {
    const tokenName = node.registration.token.name;
    
    if (node.status === 'failed') {
      throw new Error(`Service creation failed previously: ${tokenName}. Error: ${node.error?.message}`);
    }
  ...
  private validateNonCircularDependencies(tokenName: string, dependencies: ServiceToken<any>[]): void {
    const visited = new Set<string>();
    const visiting = new Set<string>();
