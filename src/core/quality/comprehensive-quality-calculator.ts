...
import { ESLintResult, PrettierResult, TypeScriptResult } from './async-tool-integration-manager.js';
...
export interface QualityRecommendation {
  id: string;
  category: 'complexity' | 'maintainability' | 'linting' | 'formatting' | 'typescript' | 'documentation' | 'security' | 'performance';
...
export class ComprehensiveQualityCalculator {
  ...
  async calculateQualityMetrics(
    ...
    try {
      ...
      const maintainabilityScore = this.normalizeMaintainabilityScore(astMetrics.maintainabilityIndex);
      ...
      const technicalDebtRatio = this.calculateTechnicalDebtRatio(
        astMetrics, eslintResults, prettierResults, typescriptResults
      );
      const technicalDebtMinutes = this.estimateTechnicalDebtTime(technicalDebtRatio, astMetrics.linesOfCode);
      ...
      const recommendations = this.generateIntelligentRecommendations(
        astMetrics, eslintResults, prettierResults, typescriptResults,
        { complexityScore, maintainabilityScore, lintingScore, formattingScore, typeScriptScore, documentationScore, securityScore }
      );
      
      // Calculate trend data if identifier provided
      const trendData = identifier ? this.calculateTrendAnalysis(identifier, overallScore) : undefined;
      ...
      logger.debug(`Quality calculation completed in ${qualityMetrics.analysisTime.toFixed(2)}ms with score ${overallScore}`);
      
      return qualityMetrics;
      
    } catch (error) {
      logger.error('Quality calculation failed:', error);
      throw new Error(`Quality calculation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private normalizeMaintainabilityScore(maintainabilityIndex: number): number {
    ...
    if (maintainabilityIndex >= maintainability.excellent) {
      ...
      return 70 + (maintainabilityIndex - maintainability.acceptable) / (maintainability.excellent - maintainability.acceptable) * 30;
    } else if (maintainabilityIndex >= maintainability.problematic) {
      return 40 + (maintainabilityIndex - maintainability.problematic) / (maintainability.acceptable - maintainability.problematic) * 30;
    } else if (maintainabilityIndex >= maintainability.critical) {
      return 10 + (maintainabilityIndex - maintainability.critical) / (maintainability.problematic - maintainability.critical) * 30;
    } else {
   ...
   */
  private calculateSecurityScore(astMetrics: ASTComplexityMetrics, eslintResults: ESLintResult): number {
    let score = 100;
    
    // Deduct points for security-related ESLint errors
    const securityRules = [
      'no-eval', 'no-implied-eval', 'no-new-func',
      'no-script-url', 'no-unsafe-innerHTML', 'no-danger'
    ];
  ...
  private calculateCompositeScore(scores: {
    ...
    return scores.complexityScore * weights.complexity +
  ...
  private calculateTechnicalDebtRatio(
    ...
    const complexityDebt = Math.max(0, 
      (astMetrics.cyclomaticComplexity - this.config.complexity.cyclomaticLow) * technicalDebt.complexityMultiplier +
      (astMetrics.cognitiveComplexity - this.config.complexity.cognitiveLow) * technicalDebt.complexityMultiplier * 0.5
    );
    ...
    const maintenanceDebt = Math.max(0, 
      (this.config.maintainability.acceptable - astMetrics.maintainabilityIndex) * technicalDebt.maintenanceMultiplier
    );
    
    const totalDebt = complexityDebt + lintingDebt + formattingDebt + typescriptDebt + maintenanceDebt;
   ...
   */
  private calculateQualityGrade(score: number): 'A+' | 'A' | 'B+' | 'B' | 'C+' | 'C' | 'D+' | 'D' | 'F' {
  ...
  private generateIntelligentRecommendations(
    ...
    if (astMetrics.cyclomaticComplexity > this.config.complexity.cyclomaticCritical) {
      recommendations.push({
        ...
        suggestion: 'Break down complex functions using Extract Method refactoring. Consider implementing Strategy or Command patterns to reduce decision points.',
        rationale: 'High cyclomatic complexity makes code difficult to test, debug, and maintain, increasing defect probability.',
        estimatedImpact: 30,
        effortEstimate: 'high',
        riskLevel: 'medium',
        automated: false,
      ...
      recommendations.push({
        ...
        suggestion: 'Identify and extract complex conditional logic into separate functions or use polymorphism to replace switch statements.',
        ...
        technicalDebtReduction: (astMetrics.cyclomaticComplexity - this.config.complexity.cyclomaticMedium) * 1.5
      });
    }
    ...
    if (astMetrics.cognitiveComplexity > this.config.complexity.cognitiveHigh) {
      recommendations.push({
        ...
        suggestion: 'Reduce nested conditions and loops. Consider early returns, guard clauses, or breaking logic into smaller functions.',
        rationale: 'High cognitive complexity makes code harder for humans to understand and mentally process.',
        estimatedImpact: 20,
        effortEstimate: 'medium',
        riskLevel: 'low',
        automated: false,
    ...
    if (astMetrics.maintainabilityIndex < this.config.maintainability.critical) {
      recommendations.push({
        ...
        suggestion: 'Comprehensive refactoring needed: reduce complexity, increase documentation, and improve code structure.',
        rationale: 'Critical maintainability issues indicate code that is extremely difficult and expensive to maintain.',
        estimatedImpact: 40,
        effortEstimate: 'high',
        riskLevel: 'high',
        automated: false,
        ...
        technicalDebtReduction: (this.config.maintainability.acceptable - astMetrics.maintainabilityIndex) * 3
      });
    }
    ...
    if (eslintResults.totalErrors > 0) {
      ...
      recommendations.push({
        ...
        suggestion: eslintResults.fixableIssues > 0 
        ...
        rationale: 'ESLint errors indicate potential bugs, security issues, or style inconsistencies.',
        estimatedImpact: Math.min(25, eslintResults.totalErrors * 2),
        effortEstimate: eslintResults.fixableIssues > eslintResults.totalErrors * 0.7 ? 'low' : 'medium',
        riskLevel: 'low',
    ...
    if (!prettierResults.isFormatted && prettierResults.formattingIssues > 5) {
      recommendations.push({
        ...
        suggestion: 'Run Prettier to automatically format code and ensure consistent style across the project.',
        rationale: 'Consistent formatting improves code readability and reduces diff noise in version control.',
        estimatedImpact: 10,
        effortEstimate: 'low',
        riskLevel: 'low',
        automated: true,
    ...
    if (typescriptResults.totalErrors > 0) {
      recommendations.push({
        ...
        suggestion: 'Fix TypeScript type errors to improve type safety and catch potential runtime errors.',
        rationale: 'TypeScript errors indicate type mismatches that could lead to runtime failures.',
    ...
    if (astMetrics.commentRatio < 15) {
      recommendations.push({
        ...
        suggestion: 'Add JSDoc comments to functions and classes. Document complex business logic and edge cases.',
        rationale: 'Good documentation improves code maintainability and helps new team members understand the codebase.',
        estimatedImpact: 8,
        effortEstimate: 'medium',
        riskLevel: 'low',
        automated: false,
    ...
    if (scores.securityScore < 80) {
      recommendations.push({
        ...
        suggestion: 'Review code for security anti-patterns: avoid eval(), validate inputs, use secure coding practices.',
   ...
   */
  private calculateTrendAnalysis(identifier: string, currentScore: number): QualityTrend | undefined {
    ...
    const improvementRate = scores.length > 1
      ? this.calculateLinearTrend(scores)
      : 0;
   ...
   */
  private identifyRegressionCause(previousMetrics: ComprehensiveQualityMetrics, currentScore: number): string {
