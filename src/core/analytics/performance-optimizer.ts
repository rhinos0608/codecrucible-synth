...
export class PerformanceOptimizer {
  ...
  private analyzeOperationPerformance(operation: string, metric: PerformanceMetric): void {
    ...
    if (metric.successRate < this.optimizationThresholds.lowSuccessRate && metric.count >= this.optimizationThresholds.significantSampleSize) {
  ...
  private estimateImplementationEffort(operation: string, type: string): OptimizationSuggestion['implementationEffort'] {
  ...
  private findRelevantPatterns(taskType: string, context?: any): LearningPattern[] {
    return Array.from(this.patterns.values())
      .filter(pattern => {
        ...
        if (context && pattern.metadata.tags.some(tag => 
  ...
  private analyzeModelPerformance(taskType: string, patterns: LearningPattern[]): any {
    ...
    const modelStats = new Map<string, { count: number; avgPerformance: number; successRate: number }>();
  ...
  private analyzeToolPerformance(objective: string, patterns: LearningPattern[]): any {
    ...
    const toolStats = new Map<string, { count: number; avgPerformance: number; successRate: number }>();
  ...
  private generateToolRecommendation(toolPerformance: any, objective: string): any {
    const tools = Object.entries(toolPerformance)
      .sort((a: any, b: any) => {
   ...
   * Utility methods
   */
  private extractPattern(operation: string, duration: number, success: boolean, context?: any): LearningPattern | null {
  ...
  private updateSessionAnalytics(operation: string, duration: number, success: boolean, context?: any): void {
    ...
    const totalTime = this.currentSession.averageResponseTime * (this.currentSession.totalOperations - 1) + duration;
    ...
    if (context?.voice) {
      this.currentSession.voiceUsageStats[context.voice] = (this.currentSession.voiceUsageStats[context.voice] || 0) + 1;
    }
  }
