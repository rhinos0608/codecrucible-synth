...
const testScenarios = [
  {
    ...
    command:
      'node dist/index.js "Please analyze the CodeCrucible codebase and suggest 3 specific improvements. For each improvement, provide: 1) What needs to be changed, 2) Why it\'s important, 3) Specific implementation steps, 4) Expected impact. Focus on architecture, performance, or code quality."',
  },
  {
    name: 'Multi-File Refactoring Task',
    description: 'Complex refactoring requiring coordination across multiple files',
    command:
      'node dist/index.js "I need to refactor the CLI system to improve startup time. Please: 1) Analyze the current CLI implementation, 2) Identify performance bottlenecks, 3) Create a detailed refactoring plan, 4) Show how to implement lazy loading, 5) Provide before/after performance estimates."',
  },
  {
    name: 'Living Spiral Methodology Test',
    description: 'Test the 5-phase Living Spiral process with a real problem',
    command:
      'node dist/index.js "Using the Living Spiral methodology, help me design a new feature: Real-time collaboration for multiple developers working on the same codebase. Go through all 5 phases: Collapse, Council, Synthesis, Rebirth, and Reflection."',
  },
];
...
function calculateQualityScore(output) {
  ...
  if (output.includes('conclusion') || output.includes('summary') || output.includes('next steps'))
    score += 1;
...
function generateReport() {
  ...
  console.log('\n📊 SUMMARY:');
  console.log(
    `  Tests Passed: ${successCount}/${testResults.length} (${((successCount / testResults.length) * 100).toFixed(1)}%)`
  );
  ...
  console.log(
    `  Multi-Step Decomposition: ${hasMultiStep}/${testResults.length} tests (${((hasMultiStep / testResults.length) * 100).toFixed(1)}%)`
  );
  console.log(
    `  Context Maintenance: ${hasContext}/${testResults.length} tests (${((hasContext / testResults.length) * 100).toFixed(1)}%)`
  );
  console.log(
    `  Implementation Details: ${hasImplementation}/${testResults.length} tests (${((hasImplementation / testResults.length) * 100).toFixed(1)}%)`
  );

  const overallScore =
    ((successCount + hasMultiStep + hasContext + hasImplementation) / (testResults.length * 4)) *
    100;
