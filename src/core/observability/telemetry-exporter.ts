...
export class TelemetryExporter {
  ...
  async initialize(): Promise<void> {
    ...
    this.logger.info(`TelemetryExporter initialized with ${this.config.exporters.length} exporters`);
  }
  ...
  private async exportToFile(exporter: any, metrics: MetricPoint[]): Promise<void> {
    const filePath = this.fileHandles.get(exporter.type) || exporter.endpoint || './telemetry/metrics.jsonl';
