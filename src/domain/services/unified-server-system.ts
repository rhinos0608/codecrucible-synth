...
export class HTTPServerStrategy extends EventEmitter implements IServerStrategy {
  ...
  getStatus(): ServerStatus {
    ...
    this.status.performance.avgResponseTime = this.requestCount > 0 
...
export class UnifiedServerSystem extends EventEmitter {
  ...
  async stopAllServers(): Promise<void> {
    const stopPromises = Array.from(this.activeServers).map(type => this.stopServer(type));
  ...
  private setupEventHandlers(): void {
    ...
    this.eventBus.on('system:shutdown', () => this.stopAllServers());
...
export class ServerMode extends UnifiedServerSystem implements ServerModeInterface {
  ...
  getStatus(): ServerStatus {
    return this.getServerStatus('http') || {
      running: false,
      type: 'http',
      connections: 0,
      requestsProcessed: 0,
