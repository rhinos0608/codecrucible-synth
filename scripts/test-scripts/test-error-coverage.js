...
async function testErrorHandlingCoverage() {
  ...
  try {
    ...
    try {
      ...
      results.total++;
      if (
        outputCaptured.includes('Suggested actions:') ||
        outputCaptured.includes('Setup Guide:')
      ) {
        results.graceful++;
  ...
  if (gracefulRate >= target) {
    ...
    console.log(
      `✅ Achieved ${gracefulRate.toFixed(1)}% graceful failure rate (target: ${target}%)`
    );
    console.log('');
