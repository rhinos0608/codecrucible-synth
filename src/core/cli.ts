...
\`\`\``;
  ...
  private async handleSlashCommand(command: string, options: CLIOptions): Promise<void> {
    // SECURITY FIX: Sanitize and validate slash command input
    const sanitizationResult = InputSanitizer.sanitizeSlashCommand(command);
    
    if (!sanitizationResult.isValid) {
      const securityError = InputSanitizer.createSecurityError(sanitizationResult, 'slash-command');
      this.logger.error('Security violation in slash command', securityError);
      console.error(chalk.red('🔒 Security Error: Invalid command detected'));
      console.error(chalk.red(`Violations: ${sanitizationResult.violations.join(', ')}`));
      return;
    }

    // Use sanitized command
    const sanitizedCommand = sanitizationResult.sanitized;
    ...
    switch (cmd) {
      ...
      case '/voice':
        ...
        const rawVoiceNames = voiceParts[0].split(',');
        ...
        // SECURITY FIX: Sanitize voice names and prompt
        const sanitizedVoiceNames = InputSanitizer.sanitizeVoiceNames(rawVoiceNames);
        const promptSanitization = InputSanitizer.sanitizePrompt(voicePrompt);
        
        if (sanitizedVoiceNames.length === 0) {
          console.log(chalk.red('No valid voice names provided'));
          return;
        }
        
        if (!promptSanitization.isValid) {
          console.log(chalk.yellow('⚠️  Prompt contains potentially unsafe content - filtered for security'));
          if (options.verbose) {
            console.log(chalk.gray(`Violations: ${promptSanitization.violations.join(', ')}`));
          }
        }
        
        console.log(chalk.cyan(`🎭 Using voices: ${sanitizedVoiceNames.join(', ')}`));
        const result = await this.context.voiceSystem.synthesize(
          promptSanitization.sanitized,
          sanitizedVoiceNames,
          'collaborative',
          this.context.modelClient
        );
  ...
  async executePromptProcessing(prompt: string, options: CLIOptions): Promise<void> {
    try {
      ...
      if (!promptSanitization.isValid) {
        console.log(chalk.yellow('⚠️  Input contains potentially unsafe content - filtered for security'));
        if (options.verbose) {
          console.log(chalk.gray(`Violations: ${promptSanitization.violations.join(', ')}`));
        }
        this.logger.warn('Prompt sanitization applied', { 
          violations: promptSanitization.violations,
          originalLength: prompt.length,
          sanitizedLength: promptSanitization.sanitized.length
        });
      }

      // Use sanitized prompt for processing
      const cleanPrompt = promptSanitization.sanitized.trim();
      ...
      // Check for quoted slash commands (Windows quotes handling) - with additional security
  ...
  async destroy(): Promise<void> {
    try {
      ...
      // MEMORY LEAK FIX: Clean up streaming client and EventEmitter resources
      if (this.streamingClient) {
        // Remove all listeners and clean up streaming client
        if (typeof (this.streamingClient as any).removeAllListeners === 'function') {
          (this.streamingClient as any).removeAllListeners();
        }
        if (typeof (this.streamingClient as any).destroy === 'function') {
          await (this.streamingClient as any).destroy();
        }
      }

      // Clean up context-aware CLI resources
      if (this.contextAwareCLI) {
        if (typeof (this.contextAwareCLI as any).removeAllListeners === 'function') {
          (this.contextAwareCLI as any).removeAllListeners();
        }
        if (typeof (this.contextAwareCLI as any).shutdown === 'function') {
          (this.contextAwareCLI as any).shutdown();
        }
      }
      ...
      // Clean up auto-configurator
      if (this.autoConfigurator) {
        if (typeof (this.autoConfigurator as any).removeAllListeners === 'function') {
          (this.autoConfigurator as any).removeAllListeners();
        }
        if (typeof (this.autoConfigurator as any).shutdown === 'function') {
          (this.autoConfigurator as any).shutdown();
        }
      }
