...
export class IntelligentBatchProcessor extends EventEmitter {
   ...
   */
  async queueRequest(prompt: string, options: any = {}, priority: string = 'medium'): Promise<any> {
  ...
  private async processBatch(batchKey: string): Promise<void> {
    ...
    try {
      ...
      logger.info(
        `Batch ${batchId} completed in ${processingTime}ms (${successCount}/${results.length} successful)`
      );
    } catch (error) {
  ...
  getStatus(): {
    ...
    const queuedRequests = Array.from(this.batchQueues.values()).reduce(
