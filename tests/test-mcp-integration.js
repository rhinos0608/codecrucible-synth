...
} from './dist/core/tools/enhanced-tool-integration.js';
import {
  getGlobalToolIntegration,
  initializeGlobalToolIntegration,
} from './dist/core/tools/tool-integration.js';
...
async function testMCPIntegration() {
  try {
    ...
    console.log(
      `   Enhanced Tool Integration: ${enhancedIntegration ? '✅ CREATED' : '❌ STILL NULL'}`
    );
    ...
    if (availableToolNames && availableToolNames.length > 0) {
      console.log(
        `   Tool Names: ${availableToolNames.slice(0, 5).join(', ')}${availableToolNames.length > 5 ? '...' : ''}`
      );
    }
    ...
    if (enhancedToolNames && enhancedToolNames.length > 0) {
      console.log(
        `   Enhanced Tool Names: ${enhancedToolNames.slice(0, 5).join(', ')}${enhancedToolNames.length > 5 ? '...' : ''}`
      );
    }
    ...
    const hasReadFileTool = enhancedLLMFunctions.some(
      t => t.function?.name === 'mcp_read_file' || t.name === 'mcp_read_file'
    );
    ...
    console.log('===========================');
    console.log(
      `Basic Integration: ${basicIntegration ? '✅ Working' : '❌ Failed'} (${llmFunctions?.length || 0} tools)`
    );
    console.log(
      `Enhanced Integration: ${enhancedIntegration ? '✅ Working' : '❌ Failed'} (${enhancedLLMFunctions?.length || 0} tools)`
    );
    console.log(
      `Global Access: ${simulatedEnhancedIntegration ? '✅ Available' : '❌ Not Available'}`
    );
