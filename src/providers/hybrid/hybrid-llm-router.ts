...
export class HybridLLMRouter extends EventEmitter {
  ...
  private analyzePromptPatterns(prompt: string): { complexityBoost: number; patterns: string[] } {
    ...
    const highComplexityPatterns = [
      {
        pattern: /\b(analyze|review|evaluate|assess|investigate)\b/gi,
        weight: 0.3,
        name: 'deep-analysis',
      },
      {
        pattern: /\b(secure|security|vulnerability|exploit|attack)\b/gi,
        weight: 0.35,
        name: 'security',
      },
      {
        pattern: /\b(architecture|design|structure|framework|pattern)\b/gi,
        weight: 0.3,
        name: 'architectural',
      },
      {
        pattern: /\b(optimize|performance|efficiency|scalable?|benchmark)\b/gi,
        weight: 0.25,
        name: 'optimization',
      },
      {
        pattern: /\b(complex|complicated|intricate|sophisticated)\b/gi,
        weight: 0.2,
        name: 'complexity-indicator',
      },
      {
        pattern: /\b(algorithm|data structure|computational)\b/gi,
        weight: 0.3,
        name: 'algorithmic',
      },
    ];
    ...
    [...highComplexityPatterns, ...mediumComplexityPatterns, ...lowComplexityPatterns].forEach(
      ({ pattern, weight, name }) => {
    ...
    const technicalTerms = prompt.match(
      /\b(database|API|protocol|algorithm|encryption|authentication|authorization)\b/gi
    );
    if (technicalTerms) {
