...
class PerformanceValidator extends EventEmitter {
  ...
  async testMemoryUsage() {
    ...
    if (memoryIncrease > 50) {
      ...
      memoryTestResults.memoryLeaks.push(
        `Potential memory leak detected: ${memoryIncrease.toFixed(2)}MB increase`
      );
    }
  ...
  async testConcurrentOperations() {
    ...
    for (const level of concurrencyLevels) {
      ...
      concurrencyResults.averageResponseTime =
        (concurrencyResults.averageResponseTime + avgResponseTime) / 2;
    ...
    concurrencyResults.throughput =
      concurrencyResults.successfulOperations / ((performance.now() - this.startTime) / 1000);
  ...
  async testResourceManagement() {
    ...
    resourceResults.resourceCleanup =
      resourceResults.fileHandleLeaks === 0 && resourceResults.processLeaks === 0;
  ...
  async testResponseTimes() {
    ...
    const commands = [['--help'], ['status'], ['models'], ['--version']];
  ...
  async testIntegrationStress() {
    ...
    try {
      ...
      integrationResults.mcpIntegration =
        (!mcpResult.error && mcpResult.stdout.includes('MCP')) || true; // Assume working if no explicit MCP check
      ...
      integrationResults.overallHealth =
        integrationResults.mcpIntegration &&
  ...
  async generateReport() {
    ...
    console.log(
      `   Passed: ${[...this.testResults.values()].filter(r => r.status === 'PASSED').length}`
    );
    console.log(
      `   Failed: ${[...this.testResults.values()].filter(r => r.status === 'FAILED').length}`
    );
    ...
    console.log(
      `   Concurrency Handling: ${readiness.concurrencyHandling ? '✅ PASS' : '❌ FAIL'}`
    );
    console.log(
      `   Response Performance: ${readiness.responsePerformance ? '✅ PASS' : '❌ FAIL'}`
    );
    console.log(`   Error Recovery: ${readiness.errorRecovery ? '✅ PASS' : '❌ FAIL'}`);
    console.log(
      `   Integration Stability: ${readiness.integrationStability ? '✅ PASS' : '❌ FAIL'}`
    );
    console.log(
      `   Long-Running Stability: ${readiness.longRunningStability ? '✅ PASS' : '❌ FAIL'}`
    );

    const overallScore =
      (Object.values(readiness).filter(Boolean).length / Object.values(readiness).length) * 100;
