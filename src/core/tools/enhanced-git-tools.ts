...
export class GitOperationsTool extends BaseTool {
  constructor(private agentContext: { workingDirectory: string }) {
    ...
    super({
      name: 'gitOperations',
      description:
        'Comprehensive Git operations: status, commit, branch, merge, and all Git functionality',
    ...
    try {
      ...
      if (!(await this.isGitRepository())) {
        if (args.operation !== 'init' && args.operation !== 'clone') {
          return {
            error:
              'Not in a Git repository. Use "init" to initialize or "clone" to clone a repository.',
          };
        }
      }
...
export class GitAnalysisTool extends BaseTool {
  ...
  private async getBranchAnalysis(): Promise<any> {
    try {
      ...
      return {
        error: `Failed to analyze branches: ${error instanceof Error ? error.message : 'Unknown error'}`,
      };
    }
  }
  ...
  private async getContributorAnalysis(since?: string, until?: string): Promise<any> {
    try {
      ...
      const contributors = result.stdout
        .split('\n')
      ...
      return {
        error: `Failed to analyze contributors: ${error instanceof Error ? error.message : 'Unknown error'}`,
      };
    }
  }
  ...
  private async getFileHistory(file: string): Promise<any> {
    try {
      const [history, stats] = await Promise.all([
        execAsync(`git log --oneline --follow -- "${file}"`, {
          cwd: this.agentContext.workingDirectory,
        }),
        execAsync(`git log --numstat --follow -- "${file}"`, {
          cwd: this.agentContext.workingDirectory,
        }),
      ]);
  ...
  private async getCommitStatistics(author?: string, since?: string, until?: string): Promise<any> {
    try {
      ...
      return {
        error: `Failed to get commit statistics: ${error instanceof Error ? error.message : 'Unknown error'}`,
      };
    }
  }
  ...
  private async getConflictAnalysis(): Promise<any> {
    try {
      ...
      return {
        error: `Failed to analyze conflicts: ${error instanceof Error ? error.message : 'Unknown error'}`,
      };
    }
  }
  ...
  private async getRemoteAnalysis(): Promise<any> {
    try {
      ...
      const remoteInfo = remotes.stdout
        .split('\n')
        .filter(line => line.trim())
        .map(line => {
      ...
      return {
        error: `Failed to analyze remotes: ${error instanceof Error ? error.message : 'Unknown error'}`,
      };
    }
  }
  ...
  private async getTagAnalysis(): Promise<any> {
    try {
      ...
      return {
        error: `Failed to analyze tags: ${error instanceof Error ? error.message : 'Unknown error'}`,
      };
    }
  }
  ...
  private async getBlameAnalysis(file: string): Promise<any> {
    try {
      ...
      return {
        error: `Failed to get blame analysis: ${error instanceof Error ? error.message : 'Unknown error'}`,
      };
    }
  }
}
