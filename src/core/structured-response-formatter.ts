...
export interface FormattedSection {
  title: string;
  content: string;
  type: 'header' | 'info' | 'warning' | 'error' | 'success' | 'table' | 'list' | 'code' | 'markdown';
  priority: number;
}
...
export class StructuredResponseFormatter {
  ...
  private createArchitectureSection(projectIndex: ProjectIndex): FormattedSection {
    ...
    content.push(`${figures.pointerSmall} Entry Points: ${projectIndex.structure.entryPoints.length}`);
    content.push(`⚙️ Config Files: ${projectIndex.structure.configFiles.length}`);
    content.push(`${figures.circleFilled} Test Dirs: ${projectIndex.structure.testDirectories.length}`);
  ...
  private createQualityMetricsSection(projectIndex: ProjectIndex): FormattedSection {
    ...
    const scoreIcon = qualityScore >= 80 ? logSymbols.success : qualityScore >= 60 ? logSymbols.warning : logSymbols.error;
    ...
    const documentationCoverage = Math.round((metrics.coverage.documented / metrics.coverage.total) * 100);
    const testCoverage = Math.round((metrics.coverage.tested / metrics.coverage.total) * 100);
    
    table.push(
      ['Documentation', `${documentationCoverage}%`, this.getStatusIcon(documentationCoverage, 70, 50)],
      ['Test Coverage', `${testCoverage}%`, this.getStatusIcon(testCoverage, 80, 60)],
      ['Avg Complexity', metrics.complexity.average.toFixed(1), this.getStatusIcon(100 - metrics.complexity.average * 2, 70, 50)],
      ['External Deps', metrics.dependencies.external.length.toString(), metrics.dependencies.external.length < 20 ? '✅' : '⚠️']
    );
  ...
  private createCodeQualitySection(code: string): FormattedSection {
    ...
    const avgLineLength = Math.round(lines.reduce((sum, line) => sum + line.length, 0) / lines.length);
    
    const content: string[] = [];
    content.push(`${figures.info} Comment Ratio: ${commentRatio}% ${this.getStatusIcon(commentRatio, 15, 5)}`);
    content.push(`📏 Avg Line Length: ${avgLineLength} chars ${this.getStatusIcon(100 - avgLineLength, 50, 30)}`);
  ...
  private createCodePatternsSection(code: string): FormattedSection {
    ...
    return {
      title: 'Code Patterns',
      content: patterns.length > 0 ? patterns.map(p => `${figures.bullet} ${p}`).join('\n') : 'No specific patterns detected',
      type: 'info',
      priority: 80
    };
  }
  ...
  private createCodeSuggestionsSection(code: string): FormattedSection {
    ...
    return {
      title: 'Suggestions',
      content: suggestions.length > 0 ? suggestions.map(s => `${figures.bullet} ${s}`).join('\n') : 'Code looks good!',
  ...
  private renderSummaryBox(summary: string, metadata: { confidence: number; processingTime: number; dataQuality: string }): string {
    const content = [
      summary,
      '',
  ...
  private generateProjectSummary(projectIndex: ProjectIndex): string {
    ...
    return `This ${metadata.type} project "${metadata.name}" contains ${metadata.totalFiles} files across ${Object.keys(metadata.languages).length} languages. ` +
