...
export interface MetricsSummary {
  ...
  aggregations: Record<string, { count: number; avg: number; min: number; max: number; p95: number; p99: number }>;
}
...
export class MetricsCollector {
  ...
  getSummary(timeRange?: { start: Date; end: Date }): MetricsSummary {
    ...
    return {
      ...
      timeRange: timeRange || { start: metrics[0]?.timestamp ?? new Date(), end: metrics[metrics.length - 1]?.timestamp ?? new Date() },
  ...
  private getAggregations(metrics: MetricPoint[]): Record<string, { count: number; avg: number; min: number; max: number; p95: number; p99: number }> {
    const aggregations: Record<string, { count: number; avg: number; min: number; max: number; p95: number; p99: number }> = {};
    for (const m of metrics) {
      if (!aggregations[m.name]) {
        aggregations[m.name] = { count: 0, avg: 0, min: Number.MAX_VALUE, max: Number.MIN_VALUE, p95: 0, p99: 0 };
      }
