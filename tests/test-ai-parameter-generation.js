...
const testScenarios = [
  ...
  },
  {
    ...
    prompt:
      'Analyze the TypeScript configuration and package.json files to understand the project structure',
    expectedTool: 'filesystem_read_file',
  ...
  },
  {
    ...
    prompt:
      'Check the project structure, read the main configuration files, and provide a development environment analysis',
    expectedTool: 'filesystem_list_directory',
    expectedParams: { path: '.' },
    taskType: 'mixed',
    domain: 'mixed',
...
async function testParameterGeneration() {
  try {
    ...
    for (const scenario of testScenarios) {
      ...
      console.log(
        `   Expected: ${scenario.expectedTool} with ${JSON.stringify(scenario.expectedParams)}`
      );
      ...
      const paramContext = {
        ...
        toolSchema: {
          ...
          required:
            scenario.expectedTool === 'filesystem_read_file'
              ? ['filePath']
              : scenario.expectedTool === 'mcp_execute_command'
                ? ['command']
                : [],
        },
      ...
      try {
        ...
        const hasExpectedParam = Object.keys(scenario.expectedParams).some(
          key => aiResult.parameters[key] !== undefined
        );
      ...
      if (
        heuristicResult.analysis.primaryDomain === scenario.domain &&
    ...
    try {
      ...
      const complexPrompt =
        'Analyze the project structure by reading the README.md and package.json files';
      const context = {
        sessionId: 'test',
      ...
      toolCalls.forEach((call, i) => {
        console.log(
          `      ${i + 1}. ${call.toolId}: ${JSON.stringify(call.input)} (priority: ${call.priority})`
        );
      });
    } catch (error) {
    ...
    console.log(
      `AI-powered successes: ${aiSuccesses}/${totalTests} (${Math.round((aiSuccesses / totalTests) * 100)}%)`
    );
    console.log(
      `Heuristic successes: ${heuristicSuccesses}/${totalTests} (${Math.round((heuristicSuccesses / totalTests) * 100)}%)`
    );
