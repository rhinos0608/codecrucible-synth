...
Be direct and helpful like Claude Code.`,
   ...
   */
  private async executeActions(originalCommand: string, response: string): Promise<void> {
    ...
    else if (lowerCommand.includes('write ') || lowerCommand.includes('create ') || lowerCommand.includes('modify ')) {
      await this.handleFileCreation(originalCommand, response);
    }
    ...
    // Fix or modify existing files
    else if (lowerCommand.includes('fix ') || lowerCommand.includes('improve ') || lowerCommand.includes('refactor ')) {
      await this.handleCodeModification(originalCommand, response);
    }
  }
  ...
  private async readFile(filepath: string): Promise<void> {
    try {
      ...
      if (!file) {
        const matches = Array.from(this.codebase.files.keys()).filter(f => 
          f.includes(filepath) || basename(f) === filepath
        );
...
`));
   ...
   */
  private buildProjectContext(): any {
  ...
  /**
   * Handle file creation or writing
   */
  private async handleFileCreation(originalCommand: string, aiResponse: string): Promise<void> {
    try {
      console.log(chalk.blue('📝 Creating/writing file...'));
      ...
      const response = await this.context.modelClient.generateVoiceResponse(
        {
          id: 'implementor',
          name: 'Implementor',
          systemPrompt: `You are a code implementation specialist. Generate complete, working code files based on requests.
...
Current project context: ${this.codebase.rootPath}`,
          temperature: 0.6,
          style: 'precise'
        },
        originalCommand,
      ...
      // Try to extract filename and code
      const fileMatch = originalCommand.match(/(?:write|create|modify)\s+([^\s]+)/i);
      const codeBlocks = response.content.match(/```[\w]*\n([\s\S]*?)\n```/g);
      
      if (fileMatch && codeBlocks) {
        ...
        const { shouldSave } = await inquirer.prompt([
          {
            type: 'confirm',
            name: 'shouldSave',
            message: `Save this code to ${filename}?`,
            default: true
          }
        ]);
        
        if (shouldSave) {
          await writeFile(join(this.codebase.rootPath, filename), code, 'utf8');
          console.log(chalk.green(`✅ Created ${filename}`));
          
          // Update our file cache
          this.codebase.files.set(filename, {
            path: filename,
            content: code,
            language: this.detectLanguage(extname(filename)),
            size: code.length
          });
        }
      }
      ...
      console.error(chalk.red(`❌ File creation failed: ${error}`));
    }
  }

  /**
   * Handle code modification for existing files
   */
  private async handleCodeModification(originalCommand: string, aiResponse: string): Promise<void> {
    try {
      console.log(chalk.blue('🔧 Modifying code...'));
      
      // Try to identify the file to modify
      const existingFiles = Array.from(this.codebase.files.keys());
      let targetFile = '';
      
      // Look for file mentions in the command
      for (const file of existingFiles) {
        if (originalCommand.toLowerCase().includes(file.toLowerCase()) || 
            originalCommand.toLowerCase().includes(basename(file).toLowerCase())) {
          targetFile = file;
          break;
        }
      }
        ...
        const { selectedFile } = await inquirer.prompt([
          {
            ...
            choices: existingFiles.slice(0, 20) // Show first 20 files
          }
        ]);
        targetFile = selectedFile;
      }
      
      const file = this.codebase.files.get(targetFile);
      if (!file) {
      ...
      const response = await this.context.modelClient.generateVoiceResponse(
        {
          id: 'implementor',
          name: 'Implementor',
          systemPrompt: `You are a code modification specialist. Modify the provided code based on the request.
          
Guidelines:
- Preserve existing functionality unless asked to change it
- Follow the existing code style
- Add helpful comments for changes
...
Request: ${originalCommand}`,
        ...
        `Modify this ${file.language} file: ${originalCommand}\n\nCurrent content:\n\`\`\`${file.language}\n${file.content}\n\`\`\``,
        { files: [{ path: file.path, content: file.content, language: file.language }] }
      );
      
      console.log(chalk.green('🔨 Modified code:'));
      
      // Extract the modified code
      const codeBlocks = response.content.match(/```[\w]*\n([\s\S]*?)\n```/g);
      if (codeBlocks) {
        const modifiedCode = codeBlocks[0].replace(/```[\w]*\n/, '').replace(/\n```$/, '');
        ...
        const { shouldApply } = await inquirer.prompt([
          {
            type: 'confirm',
            name: 'shouldApply',
            message: `Apply these changes to ${targetFile}?`,
            default: true
          }
        ]);
        
        if (shouldApply) {
          await writeFile(join(this.codebase.rootPath, targetFile), modifiedCode, 'utf8');
          console.log(chalk.green(`✅ Modified ${targetFile}`));
          
          // Update our cache
          file.content = modifiedCode;
          file.size = modifiedCode.length;
        }
      } else {
        console.log(response.content);
      }
      ...
      console.error(chalk.red(`❌ Code modification failed: ${error}`));
    }
  }
