 ...
 */
export class UnifiedSecurityValidator extends EventEmitter {
    constructor(logger, config, eventBus) {
        super();
        this.logger = logger;
    ...
    async validateCode(code, language, context) {
        const violations = [];
        const recommendations = [];
        ...
        if (this.config.allowedLanguages.length > 0 &&
    ...
    async validateFileOperation(operation, path, context) {
        const violations = [];
        const recommendations = [];
        ...
        if (this.config.allowedPaths.length > 0) {
            const isAllowed = this.config.allowedPaths.some(allowedPath => normalizedPath.startsWith(allowedPath));
            if (!isAllowed) {
                violations.push({
                    type: 'path_traversal',
                    severity: 'high',
    ...
    async validateNetworkRequest(url, method, context) {
        const violations = [];
        const recommendations = [];
    ...
    async analyzeRisk(input, context) {
    ...
    getSafeCommandAlternative(command) {
        const alternatives = {
