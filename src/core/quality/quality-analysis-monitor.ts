...
export class QualityAnalysisMonitor extends EventEmitter {
  ...
  startAnalysis(identifier?: string): string {
    ...
    if (this.activeAnalyses.size >= this.resourceLimits.maxConcurrentAnalyses) {
      throw new Error(
        `Maximum concurrent analyses (${this.resourceLimits.maxConcurrentAnalyses}) exceeded`
      );
    }
  ...
  trackComponentTime(
    ...
    component: keyof Pick<
      PerformanceMetrics,
      'astAnalysisTime' | 'lintingTime' | 'formattingTime' | 'typescriptTime' | 'calculationTime'
    >,
    duration: number
  ): void {
    const metrics = this.activeAnalyses.get(analysisId);
  ...
  completeAnalysis(
    ...
    metrics.functionsAnalyzedPerSecond =
      durationSeconds > 0 ? functionsAnalyzed / durationSeconds : 0;
    ...
    logger.info(
      `Completed analysis monitoring: ${analysisId} (${metrics.totalDuration.toFixed(2)}ms)`
    );
  ...
  getPerformanceStatus(): {
    ...
    const averageEfficiency =
      recentMetrics.length > 0
        ? recentMetrics.reduce((sum, m) => sum + m.analysisEfficiencyScore, 0) /
  ...
  getPerformanceAnalytics(): {
    ...
    const averageAnalysisTime =
      history.reduce((sum, m) => sum + m.totalDuration, 0) / history.length;
    const peakMemoryUsage = Math.max(...history.map(m => m.peakMemoryUsage));

    // Calculate efficiency trend
    const recentEfficiency =
      history.slice(-10).reduce((sum, m) => sum + m.analysisEfficiencyScore, 0) /
      Math.min(10, history.length);
    const olderEfficiency =
      history.slice(0, -10).reduce((sum, m) => sum + m.analysisEfficiencyScore, 0) /
      Math.max(1, history.length - 10);
  ...
  private calculateOverallProgress(
    ...
    const stages: AnalysisProgressInfo['stage'][] = [
      'starting',
      'ast',
      'linting',
      'formatting',
  ...
  private calculateEfficiencyScore(metrics: PerformanceMetrics): number {
    ...
    if (totalComponentTime > 10000) {
      // 10 seconds
      score -= 20;
    } else if (totalComponentTime > 5000) {
      // 5 seconds
      score -= 10;
    }

    // Penalize high memory usage
    if (metrics.peakMemoryUsage > 300) {
      // 300MB
      score -= 15;
    } else if (metrics.peakMemoryUsage > 200) {
      // 200MB
      score -= 8;
    }
    ...
    if (metrics.totalDuration > this.resourceLimits.maxAnalysisTimeMs * 0.8) {
      this.createAlert(
        ...
        `Analysis took longer than expected: ${metrics.totalDuration}ms`,
        {
          ...
          threshold: this.resourceLimits.maxAnalysisTimeMs * 0.8,
        }
      );
    }
  ...
  private identifyBottlenecks(history: PerformanceMetrics[]): string[] {
    ...
    const avgFormattingTime =
      history.reduce((sum, m) => sum + m.formattingTime, 0) / history.length;
    const avgTypescriptTime =
      history.reduce((sum, m) => sum + m.typescriptTime, 0) / history.length;
