...
export class CLI {
     ...
     */
    async displayStreamingResponse(request) {
        let buffer = '';
    ...
    async initializeContextAwareness() {
        ...
        const result = await this.resilientWrapper.executeSafely(async () => {
...
\`\`\``;
    ...
    async executePromptProcessing(prompt, options) {
        try {
            ...
            if (options.voices) {
                ...
                console.log(chalk.cyan(`Using voices: ${voiceNames.join(', ')}`));
                try {
                    const result = await this.context.voiceSystem.synthesize(enhancedPrompt, voiceNames, 'collaborative', this.context.modelClient);
                    this.displayResults(result, result.responses || []);
     ...
     */
    async showProjectIntelligence(options) {
        try {
     ...
     */
    async showSmartSuggestions(context, options) {
        try {
            const intelligence = this.contextAwareCLI.getProjectIntelligence();
            if (!intelligence) {
            ...
            if (context) {
                const contextOptions = { enableIntelligence: true };
     ...
     */
    async executeDualAgentGeneration(prompt, options) {
     ...
     */
    async executeStreamingDualAgent(prompt, options) {
     ...
     */
    async initializeDualAgentSystem() {
        try {
     ...
     * Execute code audit
     */
    async executeCodeAudit(input, options) {
        ...
        try {
            ...
            const result = await this.dualAgentSystem.generateWithAudit(`Audit this code for security, performance, and quality:\n\n${codeToAudit}`, { auditOnly: true });
     ...
     */
    async handleAnalyze(files = [], options = {}) {
        try {
            if (files.length === 0) {
     ...
     */
    async getAnalyzableFiles(dirPath, maxDepth) {
        const patterns = [
    ...
    async getBestAvailableModel() {
        try {
            ...
            const codingModels = models.filter(model => model.name?.includes('code') ||
