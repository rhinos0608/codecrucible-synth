...
export class ProductionResourceEnforcer extends EventEmitter {
  ...
  private async preExecutionCheck(context: OperationResourceContext): Promise<void> {
    ...
    if (this.emergencyMode) {
      if (context.priority < 80) {
        // Only high-priority operations in emergency
        throw new ResourceEnforcementError(
          'System in emergency mode - operation rejected',
          ResourceType.MEMORY
        );
      }
    }
    ...
    if (this.activeOperations.size >= this.limits.concurrency.maxConcurrentOperations) {
      if (this.operationQueue.length >= this.limits.concurrency.maxQueueSize) {
        throw new ResourceEnforcementError(
          'Concurrency and queue limits exceeded',
  ...
  private async queueOperation(context: OperationResourceContext): Promise<void> {
    ...
    const insertIndex = this.operationQueue.findIndex(op => op.priority < context.priority);
    ...
    logger.info(
      `Operation queued: ${context.operationId} (queue size: ${this.operationQueue.length})`
    );
  ...
  private async allocateMemory(operationId: string, amount: number): Promise<void> {
    ...
    if (currentUsage + amount > this.limits.memory.hardLimit) {
      ...
      if (newUsage + amount > this.limits.memory.hardLimit) {
        throw new ResourceEnforcementError(
          'Memory allocation would exceed hard limit',
          ResourceType.MEMORY
        );
      }
    }
  ...
  private async releaseResourceAmount(
    resourceType: ResourceType,
    operationId: string,
    amount: number
  ): Promise<void> {
  ...
  private async killLowPriorityOperations(): Promise<void> {
    ...
    for (const context of lowPriorityOps.slice(0, 5)) {
      // Kill up to 5 lowest priority
      context.state = 'failed';
      await this.releaseResources(context);
  ...
  private async releaseUnusedResources(): Promise<void> {
    ...
    const staleOperations = Array.from(this.activeOperations.values()).filter(
      op => op.state === 'completed' || op.state === 'failed'
    );
  ...
  private getActiveCPUOperations(): number {
    return Array.from(this.activeOperations.values()).filter(op => op.resourceRequirements.cpu)
      .length;
  }
