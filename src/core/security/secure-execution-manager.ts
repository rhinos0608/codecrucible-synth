...
export class SecureExecutionManager {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      if (this.config.enforceE2BOnly) {
        throw new Error('SECURITY CRITICAL: Cannot initialize E2B service. Code execution disabled for security.');
      }
   ...
   */
  private async executeViaE2B(request: ExecutionRequest, startTime: number): Promise<ExecutionResult> {
    try {
      ...
      logger.info(`✅ E2B execution completed - Success: ${result.success}, Time: ${result.executionTime}ms`);
   ...
   */
  private async validateExecutionSecurity(request: ExecutionRequest): Promise<{ isValid: boolean; reason?: string; severity?: string }> {
    try {
      // Basic input validation
      if (!request.command || typeof request.command !== 'string') {
        return { isValid: false, reason: 'Invalid command: must be a non-empty string', severity: 'high' };
      }

      if (request.command.length > 10000) {
        return { isValid: false, reason: 'Command too long (max 10,000 characters)', severity: 'high' };
      }
  ...
  private isSafeEnvironmentVariable(key: string, value: string): boolean {
    ...
    if (key.startsWith('_') || key.includes('PASSWORD') || key.includes('SECRET') || key.includes('TOKEN')) {
      return false;
    }
