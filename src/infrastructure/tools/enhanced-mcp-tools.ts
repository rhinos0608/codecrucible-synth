  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (globalScope.mcp__ref_tools_ref_tools_mcp__ref_search_documentation) {
        this.services.ref_search_documentation =
          globalScope.mcp__ref_tools_ref_tools_mcp__ref_search_documentation;
      }
      ...
      if (globalScope.mcp__upstash_context_7_mcp__resolve_library_id) {
        this.services.resolve_library_id =
          globalScope.mcp__upstash_context_7_mcp__resolve_library_id;
      }
      ...
      if (globalScope.mcp__gong_rzhe_office_word_mcp_server__create_document) {
        this.services.create_document =
          globalScope.mcp__gong_rzhe_office_word_mcp_server__create_document;
      }
      if (globalScope.mcp__gong_rzhe_office_word_mcp_server__get_document_text) {
        this.services.get_document_text =
          globalScope.mcp__gong_rzhe_office_word_mcp_server__get_document_text;
      }
      if (globalScope.mcp__gong_rzhe_office_word_mcp_server__add_paragraph) {
        this.services.add_paragraph =
          globalScope.mcp__gong_rzhe_office_word_mcp_server__add_paragraph;
      }
      if (globalScope.mcp__gong_rzhe_office_word_mcp_server__search_and_replace) {
        this.services.search_and_replace =
          globalScope.mcp__gong_rzhe_office_word_mcp_server__search_and_replace;
      }

      // Sequential thinking
      if (globalScope.mcp__smithery_ai_server_sequential_thinking__sequentialthinking) {
        this.services.sequentialthinking =
          globalScope.mcp__smithery_ai_server_sequential_thinking__sequentialthinking;
      }

      // Remote shell
      if (globalScope.mcp__samihalawa_remote_shell_terminal_mcp__shell_exec) {
        this.services.shell_exec =
          globalScope.mcp__samihalawa_remote_shell_terminal_mcp__shell_exec;
      }
      ...
      switch (params.action) {
        ...
        default:
          return {
            ...
            available_services: availableServices,
            service_categories: {
              research: availableServices.filter(
                s => s.includes('search') || s.includes('research')
              ),
              documentation: availableServices.filter(s => s.includes('doc') || s.includes('ref')),
              analysis: availableServices.filter(
                s => s.includes('thinking') || s.includes('analysis')
              ),
              utilities: availableServices.filter(
                s =>
                  !['search', 'research', 'doc', 'ref', 'thinking', 'analysis'].some(cat =>
                    s.includes(cat)
                  )
              ),
            },
...
export { mcpRegistry as MCPServiceRegistry };
