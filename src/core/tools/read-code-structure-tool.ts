...
export class ReadCodeStructureTool extends BaseTool {
  constructor(private agentContext: { workingDirectory: string }) {
    const parameters = z.object({
      ...
      includeContent: z.boolean().optional().default(false).describe('Include file content samples in output')
    });

    super({
      name: 'readCodeStructure',
      description: 'Autonomously read and analyze codebase structure, extracting key files, code definitions, and architectural patterns',
...
`;
        ...
        // Group by type
        const definitionsByType = structure.codeDefinitions.reduce((acc, def) => {
        ...
        for (const [type, defs] of Object.entries(definitionsByType)) {
          result += `### ${type}s:\n`;
          for (const def of defs.slice(0, 5)) { // Show top 5 per type
...
*Analysis completed autonomously - analyzed ${structure.keyFiles.length} files and extracted ${structure.codeDefinitions.length} code definitions*`;
      ...
      logger.info(`✅ ReadCodeStructureTool: Analysis complete - ${structure.keyFiles.length} files, ${structure.codeDefinitions.length} definitions`);
      return result;
