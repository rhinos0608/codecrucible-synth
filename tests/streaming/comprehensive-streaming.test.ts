  ...
  describe('Real Stream Management', () => {
    it('should start and manage streaming sessions', async () => {
      try {
        ...
        console.log(
          `⚠️ Streaming session test failed: ${error} - may indicate provider connectivity issues`
        );
    ...
    it('should handle multiple concurrent streaming sessions', async () => {
      try {
        ...
        if (validSessions.length > 0) {
          ...
          console.log(
            `✅ Concurrent streaming: ${validSessions.length} sessions managed successfully`
          );
        } else {
        ...
        try {
          ...
          if (sessionId) {
            ...
            if (session && session.metrics.backpressureEvents > 0) {
              ...
              console.log(
                `✅ Backpressure handling: ${session.metrics.backpressureEvents} events handled`
              );
            } else {
              console.log(
                '✅ Backpressure test completed (no events triggered - buffer sufficient)'
              );
            }
        ...
        if (sessionId) {
          ...
          if (collectedTokens.length > 0) {
            ...
            for (let i = 1; i < collectedTokens.length; i++) {
              ...
              expect(collectedTokens[i].timestamp).toBeGreaterThanOrEqual(
                collectedTokens[i - 1].timestamp
              );
            }
            ...
            console.log(
              `✅ Token processing: ${collectedTokens.length} tokens, ordered correctly, finished=${hasFinishedToken}`
            );
          } else {
        ...
        if (sessionId) {
          ...
          if (finalSession && finalSession.tokens.length > 0) {
            ...
            if (tokensWithMetadata.length > 0) {
              ...
              console.log(
                `✅ Token metadata: ${tokensWithMetadata.length}/${finalSession.tokens.length} tokens with metadata`
              );
            } else {
            ...
            const filteredTokens = finalSession.tokens.filter(
              token => token.content.trim().length > 1
            );
      ...
      try {
        ...
        if (sessionId) {
          ...
          if (finalSession && finalSession.tokens.length > 0) {
            ...
            console.log(
              `✅ Performance metrics: ${metrics.tokensStreamed} tokens, ${metrics.streamDuration}ms duration, ${metrics.throughput.toFixed(2)} tokens/sec`
            );
          } else {
          ...
          console.log(
            `✅ Global metrics: ${globalMetrics.totalSessions} sessions, ${globalMetrics.totalTokensStreamed} total tokens`
          );
          ...
          if (validSessions.length > 0) {
            ...
            const memoryCheckInterval = setInterval(() => {
              ...
              if (memoryIncrease > 100 * 1024 * 1024) {
                // 100MB
                console.log(
                  `Memory pressure detected: +${(memoryIncrease / 1024 / 1024).toFixed(1)}MB`
                );
              }
            }, 2000);
            ...
            console.log(
              `✅ Memory pressure handling: ${validSessions.length} streams, +${(totalIncrease / 1024 / 1024).toFixed(1)}MB memory`
            );
          } else {
  ...
  describe('Real Stream Error Handling and Recovery', () => {
    ...
    it('should cleanup resources after stream completion', async () => {
      try {
        ...
        const sessionId = await streamingManager.startStream('Short response for cleanup testing', {
          provider: 'ollama',
          temperature: 0.3,
          maxTokens: 256,
        });
        ...
        if (sessionId) {
          ...
          expect(finalMetrics.totalSessions).toBeGreaterThanOrEqual(
            initialMetrics.totalSessions + 1
          );

          console.log(
            `✅ Resource cleanup: session cleaned, ${finalMetrics.activeSessions} active sessions remaining`
          );
        } else {
